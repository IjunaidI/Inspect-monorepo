...
const presets = [
  { name: 'Standard Knit Shirt', industry: 'Garments', desc: '6 loops · ANSI Z1.4', loops: [['Fabric inspection', 4], ['Stitching & seams', 6], ['Collar & neckline', 5], ['Sleeves & cuffs', 4], ['Buttons & buttonholes', 5]] as [string, number][], extra: 1, used: 12, edited: '2 days ago' },
  { name: 'Bath Towel — 500 GSM', industry: 'Home Textiles', desc: '4 loops · weight check', loops: [['Pile uniformity', 3], ['Edge hemming', 2], ['GSM weighing', 1], ['Color & dye', 2]] as [string, number][], extra: 0, used: 5, edited: '6 hours ago' },
  { name: 'Bedding Set — King', industry: 'Home Textiles', desc: '7 loops · seam strength', loops: [['Fabric inspection', 4], ['Flat sheet seams', 5], ['Fitted corners', 4], ['Pillowcase stitching', 3], ['Embroidery', 2]] as [string, number][], extra: 2, used: 8, edited: 'Yesterday' },
  { name: 'Cotton Trouser', industry: 'Garments', desc: '5 loops · pocket integrity', loops: [['Fabric & shade', 4], ['Side seam & inseam', 6], ['Waistband', 3], ['Pockets & zipper', 5], ['Hem finishing', 3]] as [string, number][], extra: 0, used: 9, edited: '3 days ago' },
  { name: 'Denim Heavy', industry: 'Garments', desc: '6 loops · rivet check', loops: [['Fabric & weight', 3], ['Side & inseam', 6], ['Waistband & belt loops', 4], ['Rivets & hardware', 5], ['Wash & shade', 2]] as [string, number][], extra: 1, used: 4, edited: 'Last week' },
  { name: 'Curtain Panel', industry: 'Home Textiles', desc: '4 loops · pleat alignment', loops: [['Fabric drop', 3], ['Header & pleats', 4], ['Side hems', 2], ['Lining attachment', 3]] as [string, number][], extra: 0, used: 3, edited: '2 weeks ago' },
];
...
const industryTag = (industry: string): CSSProperties => {
  ...
  const c = map[industry] ?? map.Textile;
    ...
    <div style={{ padding: '28px 32px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 24, marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {([['All', 12, true], ['Garments', 5, false], ['Home Textiles', 4, false], ['Textile', 3, false]] as const).map(([l, n, a]) => (
            <div key={l} style={{ height: 30, padding: '0 14px', display: 'inline-flex', alignItems: 'center', gap: 6, border: `1px solid ${a ? ui.ink : ui.line}`, background: a ? ui.ink : '#fff', color: a ? '#fff' : ui.sub, borderRadius: 999, fontSize: 12, fontWeight: 500 }}>
              {l} <span style={{ ...monoChip, opacity: 0.7 }}>{n}</span>
            </div>
          ))}
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8, color: ui.sub, fontSize: 12.5 }}>
          <span>Sort by</span>
          <div style={{ height: 30, padding: '0 10px', border: `1px solid ${ui.line}`, borderRadius: 8, background: '#fff', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        {presets.map((p) => (
          <div key={p.name} style={card}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', borderTop: `1px solid ${ui.lineSoft}`, paddingTop: 10 }}>
              ...
              {p.extra > 0 && <div style={{ fontSize: 11.5, color: ui.faint, padding: '4px 0 0', marginLeft: 22 }}>+{p.extra} more</div>}
            </div>
