...
function makeService(options?: {
  ...
  const mail = {
    sendBuyerGuestMagicLink: jest.fn(
      async () => options?.mailResult ?? { sent: true },
    ),
  };
  const audit = { append: jest.fn(async () => ({})) };

  const service = new BuyerGuestsService(
    prisma as any,
    mail as any,
    audit as any,
  );
...
describe('BuyerGuestsService.invite', () => {
  ...
  it('rejects a missing email and sends nothing', async () => {
    ...
    await expect(
      service.invite('org1', ACTOR, 'b1', { email: ' ' } as any),
  ...
  it('rejects an unknown buyer (tenant-scoped) and sends nothing', async () => {
    ...
    await expect(
      service.invite('org1', ACTOR, 'b-nope', { email: 'g@b.com' }),
