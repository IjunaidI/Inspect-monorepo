...
export interface ReportCanonicalSnapshot {
  ...
  /** v1 party keys (reports signed before INS-055). */
  buyer?: { id?: string | null; name?: string | null } | null;
  supplier?: { id?: string | null; name?: string | null } | null;
  /** v2 party keys (INS-055). Read BOTH via readCanonicalParties, never directly. */
  canonicalVersion?: number;
  client?: { companyId?: string | null; name?: string | null } | null;
...
export async function renderReportPdf(
  ...
  // INS-055 spec §5.5: parties come from the shared reader so a v1 and a v2
  // snapshot render IDENTICALLY. Never destructure snap.buyer/snap.client here.
  const parties = readCanonicalParties(snap);
  const clientName = winAnsiSafe(parties.client.name).trim() || '—';
  drawHeaderBand(p, fonts, brand, clientName, input.reportId);
  ...
  drawTamperProofFooter(p, fonts, input, clientName);
...
function drawHeaderBand(
  ...
  const ini = initialsOf(clientName);
  p.text(ini, {
...
function drawMetaGrid(
  ...
  const pairs: Array<[string, string, boolean]> = [
    ...
    [
      'Factory',
      winAnsiSafe(readCanonicalParties(snap).factory?.name) || '—',
      false,
    ],
...
function drawTamperProofFooter(
  ...
  p.ensure(24);
  p.paragraph(
    `Generated by Inspect - ${clientName} - This report is read-only. Corrections are issued as a new linked re-inspection.`,
    { size: 8, color: FAINT },
  );
  p.paragraph(
