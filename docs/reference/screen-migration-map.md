...
> ([INS-090](../future/BACKLOG.md) deploy ✅ 2026-09-02 → real origins in `eas.json` ✅ →
...
> exchange itself.
>
> ✅ [INS-090](../future/BACKLOG.md) is done (2026-09-02): the API is reachable at
> `https://main-application-production-6fa4.up.railway.app` and `eas.json` carries the real origins.
> Every row below now waits only on the single on-device acceptance pass.
...
| `/inspections/[id]/review` | `/inspections/[id]/review` | `POST /inspections/:id/decision`, `GET /inspections/:id/populate` (evidence) | view: any · decide: `QA_MANAGER` | 4 | in-progress (built 2026-08-31: AQL result, submit-for-review, decision form, linked re-inspection; **2026-09-05:** photo evidence by unit in capture order (local-first from the device cache), submit disabled while this device still holds uploads, pull-to-refresh; device acceptance pending) | INS-086, INS-092, INS-093 |
| `/inspections/[id]/report` | `/inspections/[id]/report` | `POST /inspections/:id/report` (idempotent), `GET /inspections/:id`, `GET /reports/:id/pdf` | view: any · signed report: `QA_MANAGER` | 4 | in-progress (built 2026-09-02: live preview + signed report, QA-gated generate, Open PDF via presigned URL; **2026-09-05:** evidence grouped by unit in capture order, "Signed by" from the recorded signer (INS-089); device acceptance pending) | INS-086, INS-089 |
| `/reports` | `/reports` | `GET /reports` | `QA_MANAGER` | 4 | in-progress (built 2026-09-02: debounced search, distinct error/401/403/empty states; device acceptance pending — API deployed 2026-09-02, INS-090 done) | INS-086 |
| `/presets` | `/presets` | `GET /loop-presets` | `QA_MANAGER` | 4 | in-progress (built 2026-09-02: search, INS-081 item-count copy; archive moved to detail; Duplicate arrives with the builder; device acceptance pending — API deployed 2026-09-02, INS-090 done) | INS-086 |
| `/presets/[id]` | `/presets/[id]` | `GET /loop-presets/:id`, `DELETE /loop-presets/:id` | `QA_MANAGER` | 4 | in-progress (built 2026-09-02: loop-global tags/measurements above ordered items (position-sorted), presigned reference thumbs w/ distinct unavailable state, archive w/ confirm; device acceptance pending — API deployed 2026-09-02, INS-090 done) | INS-086 |
| `/presets/new` | `/presets/new` | `POST /loop-presets`, `GET /defect-catalog`, `POST /defect-catalog`, `GET /loop-presets/:from` | `QA_MANAGER` | 4 | in-progress (built 2026-09-02: single-pane builder — ordered items w/ up/down reorder, loop-global severity-grouped defect chips (REAL Pressables — fixes the recorded a11y gap) + custom defect creation, measurement sheet, ?from= duplicate w/ a visible failure notice, INS-076 version hint; reference-image upload deferred (expo-image-picker); device acceptance pending — API deployed 2026-09-02, INS-090 done) | INS-086 |
| `/companies/[id]` | `/companies/[id]` | `GET/PATCH/DELETE /companies/:id`, `POST /companies/:id/restore` | `QA_MANAGER` | 4 | in-progress (built 2026-09-02: edit form incl. tri-state logo remove + GPS pair rule, archive w/ confirm, archived banner + restore the web page lacks; logo UPLOAD deferred — needs expo-image-picker; device acceptance pending — API deployed 2026-09-02, INS-090 done) | INS-086 |
| `/companies/[id]/guests` | `/companies/[id]/guests` | `GET/POST /companies/:companyId/guests`, `DELETE /company-guests/:id` | `QA_MANAGER` | 4 | in-progress (built 2026-09-02: invite w/ TTL chips + one-time magic link (copy via expo-clipboard; origin from EXPO_PUBLIC_INSPECT_WEB_URL, honest fallback when unset), revoke w/ confirm, real forbidden/error states the web lacks; device acceptance pending — API deployed 2026-09-02, INS-090 done) | INS-086 |
| `/products` `/products/new` `/products/[id]` | same | `GET/POST/PATCH/DELETE /products`, `POST /products/:id/restore` | `QA_MANAGER` | 4 | in-progress (built 2026-09-02: list w/ the server q/archived/paging the WEB never used, create, edit w/ INS-074 null-clear, archive w/ confirm + archived banner + restore (dead code on the console); duplicate styleNumber now a 409 API-side; device acceptance pending — API deployed 2026-09-02, INS-090 done) | INS-086 |
| `/purchase-orders` `…/new` `…/[id]` | same | `GET/POST/PATCH/DELETE /purchase-orders` | `QA_MANAGER` | 4 | in-progress (built 2026-09-02: list, two-party create w/ shared `rankCompaniesByActivity` + self-dealing pre-check, edit (poNumber/qty only — parties immutable), delete w/ confirm; duplicate poNumber now a 409 API-side; device acceptance pending — API deployed 2026-09-02, INS-090 done; INS-091 2026-09-04: searchable pickers + inline company/product create) | INS-086 |
| `/users` | `/users` | `GET /users`, `POST /users/invite`, `PATCH /users/:id/role`, `DELETE /users/:id`, `PATCH /users/:id/reactivate` | UI: `ORG_OWNER` (list API floor is QA per INS-065) | 4 | in-progress (built 2026-09-02: roster + invite w/ link copy, role change / deactivate / reactivate w/ inline errors — never alert(); direct add-member stays web-only; device acceptance pending — API deployed 2026-09-02, INS-090 done) | INS-086 |
| `/invite` | `/invite` | `GET /invitations/:token`, `POST /invitations/accept` | public | 4 | in-progress (built 2026-09-02: deep-link token + paste fallback, 404/410 curated states, auto sign-in after accept — a mobile improvement over web's bounce-to-login; device acceptance pending — API deployed 2026-09-02, INS-090 done) | INS-086 |
...
index-keyed colours changed page-to-page — fixed on web in the same change via `hashIndex`), create via
the INS-091 quick-create sheet (name + kind, then `/companies/[id]` for the rest) with edit/archive on
`/companies/[id]`, and load-more instead of a pager.
