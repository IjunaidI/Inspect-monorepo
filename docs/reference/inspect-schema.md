...
- **Tenant-scoped uniqueness:** `@@unique([orgId, …])` for business keys (`Product.styleNumber`, `PurchaseOrder.poNumber`, `LoopPreset (name,version)`). **`Company` is the exception** (INS-055): its business key is a PARTIAL, case- and whitespace-insensitive unique index over ACTIVE rows only — see the partial-index bullet below — so archiving a company frees its name.
- **Deletion policy:** `Restrict` on every reference to immutable/historical data (Report→Inspection/Company, Inspection→Company/PO/Product, BillableEvent→Inspection, *→DefectCatalog, and **all** org-scoped FKs so an Organization is **archived, never hard-deleted**). `SetNull` on optional **actor** refs (deleting a `User` never erases audit/history). `Cascade` only for transient/draft children (`Invitation`; a draft Inspection's loops/photos/defects; preset children; report delivery/access; junctions).
...
- **Partial unique indexes** (cannot be expressed in `schema.prisma`; added in the migration because Postgres treats NULLs as distinct): unique global defect name `WHERE orgId IS NULL`; single audit `sequence` space `WHERE orgId IS NULL`; **INS-055** `companies_org_name_ci_active_key` on `(orgId, lower(btrim(name))) WHERE archivedAt IS NULL` — the company business key, case- and whitespace-insensitive over active rows only.

## 3. Model map (24 models)

**Tenancy & identity:** `Organization`, `User`, `Invitation`.
**Counterparties (INS-055):** `Company` (**one** model replacing the old `Buyer`+`Supplier` split; `kind: INTERNAL|THIRD_PARTY` is OWNERSHIP, and **trade role lives on the edge** — `clientCompanyId` / `factoryCompanyId` on `PurchaseOrder`/`Inspection`, `clientCompanyId` on `Report` — so one company can be the client on one PO and the factory on another), `CompanyGuest` (magic-link, scoped to one company's CLIENT-role reports in one org), `Product`, `PurchaseOrder`.
...
- **`orgId` alignment:** the denormalized `orgId` on children (`InspectionLoop`, `Photo`, `DefectInstance`, `AqlResult`, `CompanyGuest`) is an index/RBAC aid; the authoritative scope is the parent aggregate. The data-access layer must load children through their org-scoped parent and never trust a child `orgId` without verifying it against the parent. **Hardening follow-up (opt-in):** composite foreign keys — e.g. `@@unique([id, orgId])` on parents + `references: [id, orgId]` on children — would enforce alignment at the DB layer for the external-facing `CompanyGuest → Company` edge and the inspection-aggregate edges. Deferred to keep the MVP schema lean; flagged here so it's a conscious choice.
