...
> finally has a curated workspace) → owner invite accepted → 2 companies → product → 3-item loop preset (all
> 14 catalog defects grouped correctly by `defaultSeverity` — the INS-086 fix holds live; custom defect
> creation works) → two-party PO → inspection (AQL preview correct: lot 1000 → J/80, 0-1 · 5-6 · 7-8) →
> populate (6 real photo uploads through presigned S3 PUTs, slot advance + unit rollover, defect pinned to
> (unit 2, Back view), measurement, **the mid-cycle end gate fired correctly** naming the unit and its two
> missing items, discard worked) → auto-submit → AQL PASS → QA decision locked → **signed report generated,
> public verify `valid:true`** → guest magic link → portal shows exactly the client's report.
> **One real bug found and FIXED: the guest portal's "Download PDF" 404'd for every guest, always** —
> `portal-client.tsx` linked `/api/reports/:id/pdf`, a route that has never existed. Fixed with a new
> `app/api/guest/reports/[id]/pdf/route.ts` proxy (302 → the API's presigned URL, client identity forwarded
> so the DOWNLOAD access row records the guest) — verified live: 302 → `%PDF-` bytes. **Also fixed:** the
> populate header badge said uploads were Platform-Admin-only (stale since INS-083 — the upload itself worked
> fine as Org Owner); the branded report's hardcoded "Immutable · v1" (reports are canonical v2 since
> INS-055) and its dead `sku` field ("Style / SKU —" on every report, console and portal — no caller ever
> set it); the presets card's "N loops in this preset" (INS-081 vocabulary — it counts loop *items*); the
> public verify page's "PDF byte verification available after INS-003 lands" (INS-003 landed 2026-08-01);
> "manage users, buyers, suppliers" on /users (INS-055 sweep straggler); and a **hydration mismatch** on
> /users and the guests table (bare `toLocaleDateString()` in client components — server locale ≠ browser
> locale; now pinned to the dashboard's `en-GB` `DATE_FMT` convention). **Verified after the fixes:** web
> type-check clean · lint 0 errors (1 known font warning) · web 38/3 unchanged · every fix re-checked in the
> rendered browser. **Observations, not fixed:** populate's defect chips are plain text nodes, not buttons
> (keyboard/a11y gap — worth folding into the Phase 3 port); the builder's defect-tag *selection* doesn't
> limit populate's tag list (populate offers the whole org catalog — decide whether that's intended before
> Phase 3 copies it); after the end-gate's "Discard unit" the loop stays on populate rather than continuing
> to review. Gotcha reconfirmed: nest `--watch` restarts mid-session made one portal load fail transiently
> (generic "Unable to load reports") — retry before blaming code. **Later the same session, `gh` was
> authenticated and CI was read for the FIRST TIME — all green, full detail in handoff step 1 below.**
> Steps 2–3 of the handoff (the Railway deploy, then the EAS device build) remain user-side.
>
> Prior entry: **2026-08-31 — [INS-086](future/BACKLOG.md) Phase 2 scaffold BUILT (`apps/mobile` exists) +
...
1. **Read CI.** — ✅ **done 2026-08-31** (after `gh auth login`): **every run is green**, including the three
   from this week (the Phase 2 scaffold merge, the STATUS rebuild, and the click-through fixes `a75c34e`).
   The latest run's log confirms the full pipeline on Linux: fresh-container migrations → seed → type-check
   **11/11 tasks** (mobile included) → API unit **656/656 (42 suites)** → integration **147/147 (16 suites)**
   with `REQUIRE_STORAGE=1` (the byte spec ran, zero skips) → API build → lint **3/3, 0 errors** → the
   OpenAPI staleness gate → web build. The integration suite took ~30s on CI vs ~14 min locally — the remote
   Railway DB's latency was the whole cost — and the `audit-chain` flakiness never showed on Linux, which
   strengthens the P2028-timeout diagnosis. Two benign annotations: the actions' Node-20 deprecation notice
   and the known web font warning. Still unwritten: the CI assertion that exactly one React version resolves
   (the green builds imply the pin held, but nothing asserts it).
...
4. **Before Phase 3: click through the console once** — ✅ **done 2026-08-31** (second session): a real-
   browser end-to-end drove every screen against the live stack, found and fixed the guest-PDF 404 plus six
   stale-copy/hydration defects (see the top entry). The dev DB now has a curated workspace: **Acme Apparel
   Group** (owner `owner@acme-apparel.test`), Nordic Retail AB (client) + Delta Garments Ltd (factory),
