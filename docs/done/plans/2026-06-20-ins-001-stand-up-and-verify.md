...
> **✅ COMPLETE (2026-07-11) — INS-001 closed; INS-002 residual is user-side.** Backlog: [INS-001](../../future/BACKLOG.md) (done) + [INS-002](../../future/BACKLOG.md) (in-progress, user-side rotation). Dashboard: [../../STATUS.md](../../STATUS.md).
> The full `login → org → invite/accept → workspace CRUD → inspection → populate → submit → AQL → decision → signed report → verify` loop runs **2xx end-to-end against the Railway Postgres+Redis** via a committed smoke driver (acceptance (a)+(b), 2026-06-20). **Acceptance (c) closed 2026-07-11:** the smoke assertions are folded into a **36-test Jest integration suite** (`apps/api/test/integration/` — negative RBAC matrix, live token refresh, tamper-evidence + immutability regressions) verified green against the live Postgres, and a **GitHub Actions CI workflow** (`.github/workflows/ci.yml`) runs it against containerized Postgres 16 + Redis 7 + MinIO on every push/PR. The photo **byte** path ([INS-023](../../future/BACKLOG.md)) is exercised in CI via a presigned-PUT e2e (self-skips where MinIO is absent). Remaining, **user-side only** ([INS-002](../../future/BACKLOG.md)): rotate the previously committed Railway credentials and decide on a git-history scrub.
