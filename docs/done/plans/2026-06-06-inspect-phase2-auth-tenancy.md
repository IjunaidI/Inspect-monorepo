...
> **✅ COMPLETE (2026-07-11).** Tasks 1–4 (DB-free primitives: `rbac`/`password`/`jwt` + `docker-compose.dev.yml`) shipped and unit-tested. Tasks 5–7 (AuthModule login, guards, invite-only onboarding) **driven end-to-end against the live Railway DB** (2026-06-20 smoke loop). The three formerly-pending items closed 2026-07-11: token **refresh** verified live (round-trip + access-as-refresh/refresh-as-access rejections) and the **negative RBAC matrix** (401 garbage/forged/expired tokens, 403 role floors, cross-org 404s, no-org-admin tenant guard) covered by the [INS-009](../../future/BACKLOG.md) integration suite (`apps/api/test/integration/auth-rbac.e2e-spec.ts`, green vs the live Postgres and in CI); invite/magic-link **email** delivery implemented ([INS-004](../../future/BACKLOG.md) done — `MailService`, SMTP_URL or dev/json fallback). **Security-hardened 2026-07-11:** cross-tenant invite-accept takeover closed ([INS-035](../../future/BACKLOG.md)), JWT dev-secret fallback removed / fail-closed at boot ([INS-036](../../future/BACKLOG.md)), invitation tokens now CSPRNG ([INS-037](../../future/BACKLOG.md)), login timing equalized ([INS-042](../../future/BACKLOG.md)); rate limiting remains [INS-047](../../future/BACKLOG.md). Current state: [STATUS.md](../../STATUS.md).
>
