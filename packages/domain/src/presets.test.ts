...
const p = (id: string, name: string, version: number) => ({
  id,
  name,
  version,
  isArchived: false,
...
describe('latestPresetPerName', () => {
  ...
  it('picks the highest version whatever the input order, keeping first-appearance order of names', () => {
    ...
    expect(latestPresetPerName(shuffled).map((x) => x.id)).toEqual([
      'w1',
      'k3',
    ]);
  });
  ...
  it('pins a superseded id that is still selected (a company default), and ignores unknown ids', () => {
    expect(latestPresetPerName(all, ['k1', 'nope']).map((x) => x.id)).toEqual([
      'k3',
      'w1',
      'k1',
    ]);
    // Already shown → not duplicated.
    expect(latestPresetPerName(all, ['k3']).map((x) => x.id)).toEqual([
      'k3',
      'w1',
    ]);
  });
